...
export default async function HomePage() {
  ...
  return (
    <>
      {/* Hero premium avec dégradé animé – fond en CSS (pas d'erreur 404) */}
      <section className="relative overflow-hidden bg-gradient-to-br from-primary-700 via-primary-600 to-accent-600 text-white">
        <div className="absolute inset-0 opacity-[0.03] bg-[radial-gradient(circle_at_1px_1px,#fff_1px,transparent_0)] bg-[size:40px_40px]" />
        <div className="relative max-w-7xl mx-auto px-4 py-24 md:py-36 text-center">
          ...
          <div className="mt-10 flex flex-col sm:flex-row gap-4 justify-center">
            <Link
              href="/recherche"
              className="inline-flex items-center bg-white text-primary-700 hover:bg-gray-100 px-8 py-4 rounded-2xl font-semibold shadow-2xl shadow-black/20 transition hover:-translate-y-0.5"
            >
            ...
            <Link
              ...
              className="inline-flex items-center bg-accent-500 hover:bg-accent-600 px-8 py-4 rounded-2xl font-semibold shadow-2xl shadow-accent-500/20 transition hover:-translate-y-0.5"
            >
              Devenir prestataire
            </Link>
          </div>
            ...
            {[
              {
                ...
                desc: 'Profils validés et évalués par la communauté.',
              },
              {
                icon: <HiSearch className="w-8 h-8" />,
                title: 'Recherche facile',
                desc: 'Filtrez par catégorie et ville pour trouver le bon artisan.',
              },
              {
                icon: <HiStar className="w-8 h-8" />,
                title: 'Avis transparents',
                desc: 'Lisez les retours d’expérience des clients avant de réserver.',
              },
            ].map((item, i) => (
              <div
                key={i}
                className="bg-white rounded-3xl p-8 shadow-lg shadow-gray-200/50 hover:-translate-y-1 transition-all duration-300"
              >
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 mt-12">
            {categories.map((cat) => (
              ...
              >
                <span className="text-4xl group-hover:scale-110 transition-transform block mb-3">
                  {cat.icon}
                </span>
